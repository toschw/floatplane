...
##STEP3# Save posterior summary statistics as csv file, (remove # in each of the following lines if you want to do this this)
  ...
  write.csv(fit_summary.df, file = "STAN/flights_model/fit_summary1.csv",  quote = TRUE, sep = " ",
...
Figure2.3 <-  ggplot(sdata, aes(x=sdata$ratio,y=sdata$X50.)) +
  ...
  xlab("observed flight ratio")+ ylab("95% posterior interval for theta(j)") +
...
library(ggplot2)
library(rstan)
fig <- plot(datafit1, show_density = TRUE, pars=c("theta[322]","theta[324]","theta[518]","theta[300]","theta[309]","theta[321]","theta[530]","theta[347]","avg"), ci_level = 0.05, fill_color = "purple")
fig
...
fig <- fig + xlab("95% posterior interval for theta(j)") + guides(fill=TRUE)
fig 
# scale_y_continuous(labels=chvector[1:9],breaks=1:9) 
