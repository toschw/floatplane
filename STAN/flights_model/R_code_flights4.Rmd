...
---
#Hierarchical model using Stan for estimating the probability of introducing AIS to lake j  
...
#alternative model using rstamarm
...
Player <- data$LakeID
player_avgs <- y / n
tot_avg <- sum(y) / sum(AB)

options(mc.cores = parallel::detectCores())
SEED <- 101
wi_prior <- normal(-1, 1)  # weakly informative prior on log-odds
fit_partialpool <- 
  stan_glmer(cbind(Hits, AB - Hits) ~ (1 | Player), data = data, family = binomial("logit"),
...
```{r}
library(ggplot2)
models <- c("partial pooling")
estimates <- rbind(partialpool)
