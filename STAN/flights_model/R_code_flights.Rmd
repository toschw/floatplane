...
fig <- fig + scale_y_continuous(labels=chvector[1:12],breaks=1:12) + labs(x="95% posterior for theta(j)")
fig 
```
...
```{r}
data_sims <- extract(datafit2, permuted=TRUE)
...
#numerical test statistic using max (A) and min (B) introduction number of pilots from elodea-infested floatplane bases
...
#Figure
Shrinkage plotting observed introduction rates versus posterior medians with 95% posterior interval
```{r}
library(plyr)
library(dplyr)
library(ggplot2)
sdata <- read.csv("STAN/flights_model/fit_summary1.csv")
data <-  read.csv("STAN/floatplanedata4.csv")
...
Figure2.3 <-  ggplot(sdata2, aes(x=sdata2$rate,y=sdata2$perc50)) +
...
Figure2.3 <- Figure2.3 + geom_point() + geom_text(data=subset(sdata2,labels==1),aes(x=sdata2$rate,y=sdata2$perc50,label=names))
Figure2.3
```