...
write.csv(lakeList2, file="C:/Users/Toby/Dropbox/DATA/2015_Schwoerer_floatplane_survey/FlightPattern/lakeList2.csv")
```
...
ess_fig
ggsave("STAN/flights_model/figures/ManuscriptFigures/ess_fig.bmp", plot = last_plot(), device = "bmp", path = NULL,
  scale = 1, width = 90, height = 90, units = "mm",
  dpi = 300, limitsize = TRUE)
```
59 of the lakes showed neff/n <0.1, thus lower sampling quality there but still convergence
...
theme_set(theme_classic(base_size = 9))
con_test_fig <- mcmc_trace(fit_partialpool, pars = c("b[(Intercept) Lake:Lk_285505]","b[(Intercept) Lake:Lk_268]","b[(Intercept) Lake:Lk_243881]"),
           facet_args = list(nrow = 3)) + ggplot2::theme_update(text = element_text(size=7)) + facet_grid(. ~ model) + 

LH <- con_test_fig + xlab("iterations after warm up") + scale_y_continuous(expression(paste( alpha[j])))
LH
ggsave("STAN/flights_model/figures/ManuscriptFigures/trace_plots.tiff", plot = LH, device = "tiff", path = NULL,
  scale = 1, width = 90, height = 140, units = "mm",
  dpi = 300, limitsize = TRUE)
```

#creating an elodea Lakes table, a new variable consistent with fit object, then added to plot data
```{r}
...
#adding columns for urban source lakes
sourcelk <- c("Beck Lake","Sand Lake")
elodeaLakes <- elodeaLakes%>%
  mutate(source=LakeName %in% sourcelk)
...
#comparison plot
compare <- ggplot(plotdata, aes(x = observed, y = median, ymin = lb, ymax = ub)) +
  ...
  geom_point(data=highlight_plot, 
             aes(x=observed,y=median), 
           color='red',
             size=2.5) +
  #ggrepel::geom_text_repel(data = highlight_plot,aes(label=LakeName), hjust="inward",vjust=0)+
  facet_grid(. ~ model) +
  coord_fixed() +
...
theme_set(theme_classic(base_size = 9))
compare2 <- ggplot(subset(plotdata,model=="partial pooling"), aes(x = observed, y = median, ymin = lb, ymax = ub)) +
  geom_abline(intercept = 0, slope = 1, color = "skyblue") + 
  geom_linerange(color = "gray60", size = 0.75) + 
  geom_point(size = 2.5, shape = 21, fill = "gray30", color = "white", stroke = 0.2) + 
  geom_point(data=subset(highlight_plot,model=="partial pooling"), 
             ...
             color='red',
             size=2.5) +
  ggrepel::geom_label_repel(data = subset(highlight_plot,model=="partial pooling"),aes(label=LakeName), hjust="inward",vjust=14, size=3, point.padding = 0.5, segment.alpha=0.3)+
  #facet_grid(. ~ model) +
  coord_fixed() +
  scale_x_continuous(breaks = c(0.25,0.50,0.75,1.0)) +
  labs(x = "Observed y / n", y = expression(paste("Predicted, ",theta["j"])))
...
ggsave("STAN/flights_model/figures/ManuscriptFigures/compare.tiff", plot = compare2, device = "tiff", path = NULL,
  scale = 1, width = 140, height = 140, units = "mm",
  dpi = 300, limitsize = TRUE)
```


#Plotting posterior distributions for eldoea invaded lakes
```{r}
posterior_alphas <- as.matrix(fit_partialpool)
#transforming alphas to thetas
posterior_thetas <- 1/(1+exp(-posterior_alphas))
#creating a character vector for the elodeaLakes to be plotted except for the urban source lakes Beck and Sand Lake 
elodeaLakes

elodeaLakes2 <- elodeaLakes%>%
  filter(source=="FALSE")
  
chvector <as.vector(elodeaLakes2$modelID)
mcmc_areas(posterior_thetas,
           pars = as.vector(elodeaLakes2$modelID),
           prob = 0.8, area_method = "equal height",
           point_est="median")
```
