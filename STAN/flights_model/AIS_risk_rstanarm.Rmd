...
s_list <- s_list %>%
  select(StartID, GNIS_NAME, UniqueID)
lakeList2 <- lakeList%>%
  left_join(s_list, by=c("SeaplaneBase"="StartID"))
lakeList2 <- lakeList2 %>%
  select(LakeID, LakeName,SeaplaneBase,GNIS_NAME,UniqueID,lat,long)
lakeList2$error <- with(lakeList2,ifelse(LakeID!=UniqueID,1,0))

#creating list of errors, also taking out runways
errorlist <- lakeList2%>%
  filter(error==1 & comment!="runway")
errorlist$subset <- substr(errorlist$LakeID, 1, 1)
#list where we didn't have Lk_# and only St_#
errorlistS <- errorlist%>%
  filter(subset=="S" & UniqueID== "")
#one lake without making lake names
Stlk_53 <- errorlist%>%
  filter(LakeID=="Stlk_53")
#list where we had Lk_# 
errorlistL <- errorlist%>%
  filter(subset=="L")

#combining above
errors <- rbind(errorlistL, errorlistS,Stlk_53)

write.csv(errors, file="C:/Users/Toby/Dropbox/DATA/2015_Schwoerer_floatplane_survey/FlightPattern/errors.csv")
