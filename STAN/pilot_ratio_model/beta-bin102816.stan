...
data {
  ...
  int<lower=0> y[J]; // number of pilots from elodea-infested lakes into uninfested lake j
  int<lower=0> n[J]; // number of total pilots that fly into lake j (number of trials)
}
parameters {
...
model {
  ...
  y ~ binomial(n,theta); // likelihood for pilots
